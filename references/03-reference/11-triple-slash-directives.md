...
As of TypeScript 5.5, the compiler does not generate reference directives, and does _not_ emit handwritten triple-slash directives to output files unless those directives are marked as `preserve="true"`.
...
This directive instructs the compiler to _not_ include the default library (i.e. `lib.d.ts`) in the compilation. The impact here is similar to passing `noLib` on the command line.
